...
public class DateUtil {
    ...
    public static final String STANDARD_MM_FORMAT = "yyyy-MM-dd HH:mm";
     ...
     * HH:mm:ss 时间格式的分钟差
     */
    public static int getMinutesDiff(Date first,Date second){
        long time = first.getTime()/60000;
        long time1 = second.getTime()/60000;
        return (int) Math.abs((time-time1));
    }
    /**
     * HH:mm:ss 时间格式的分钟差
     */
    public static int getDateStrMinutesDiff(String first,String second){
        SimpleDateFormat sdf = new SimpleDateFormat("HH:mm");
        Date parseFirst = new Date();
        Date parseLast =new Date();
        try {
            parseFirst = sdf.parse(first);
            parseLast = sdf.parse(second);
        } catch (ParseException e) {
            e.printStackTrace();
        }
        long time = parseFirst.getTime()/60000;
        long time1 = parseLast.getTime()/60000;
        System.out.println(parseFirst.getTime()/60000);
        return (int) Math.abs((time-time1));
    }
