...
public abstract class UserTokenContextHolder {
    ...
    public static UserTokenVO getUserTokenVOByToken(String token) {
        TokenCache tokenCache = SpringUtil.getBean("tokenCache", TokenCache.class);

        String tokenCacheKey = ConstantPool.getCacheTokenName() + StrUtil.COLON + token;
        Object userTokenVOObj = tokenCache.get(tokenCacheKey);
        ...
        try {
            ...
            throw new BasicInfoException(BasicInfoStatusEnum.TOKEN_AUTH.getCode(), "请登录");
        }
    }
