...
import com.wky.book.config.TokenCache;
import com.wky.book.domain.BookUser;
import com.wky.book.domain.User;
import com.wky.book.domain.dto.UserDTO;
import com.wky.book.domain.dto.UserLoginDTO;
...
import com.wky.book.domain.vo.UserTokenVO;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
    ...
    public UserLoginRespVO userLogin(UserLoginReqVO userLoginReqVO) {
        ...
        String historyToken = tokenCache.get(ConstantPool.getCachePrefix() + StrUtil.COLON + CacheConstant.OPENID_KEY + openid);
        if (StrUtil.isNotBlank(historyToken)) {
            tokenCache.delete(ConstantPool.getCacheTokenName() + StrUtil.COLON + historyToken);
            tokenCache.delete(
                    ConstantPool.getCachePrefix() + StrUtil.COLON + CacheConstant.USER_SESSION_KEY + historyToken);
        }
        tokenCache.set(ConstantPool.getCachePrefix() + StrUtil.COLON + CacheConstant.OPENID_KEY + openid, userDTO.getToken());
...
//        redisTemplate.opsForValue().set(tokenCacheKey, JSONUtil.toJsonStr(userTokenVO), 30, TimeUnit.DAYS);
        tokenCache.set(tokenCacheKey, JSONUtil.toJsonStr(userTokenVO));
...
//        redisTemplate.opsForValue().set(sessionKeyCacheKey, sessionKey, 3, TimeUnit.DAYS);
        tokenCache.set(sessionKeyCacheKey, sessionKey);
