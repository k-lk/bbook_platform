...
import com.wky.book.domain.Book;
...
import com.wky.book.domain.vo.BookMoneyCountRes;
import com.wky.book.domain.vo.CategorySelectList;
import com.wky.book.domain.vo.MeMoneyCount;
import com.wky.book.domain.vo.UserMoneyStatisticsRes;
import com.wky.book.service.*;
import com.wky.book.util.MoneyConvertUtil;
import com.wky.book.domain.vo.ResultPage;
import com.wky.book.util.SnowFlakeUtil;
...
//        List<BookMoney> bookMoneyList = this.list(queryWrapper(bookMoneyQueryReqVo));
    ...
    public void addSave(BookMoney bookMoney) {
        ...
        bookMoney.setId(snowFlakeUtil.getNextId());
        this.save(bookMoney);
    }
...
//        bookService.isBookAuthor(userId);
    ...
    public List<String> findDetailedStatisticsReport(BookMoneyQueryReqVo bookMoneyQueryReqVo, HttpServletRequest request) {
        ...
        if (bookMoneyList.size() > 0) {
            // 根据支出类型设置为负数
            bookMoneyList.stream().forEach(x -> {
                if (BookMoneyTypeEnum.INCOME.getValue().equals(x.getType())) {
                    x.setMoney(-x.getMoney());
                }
            });
//            用户名称列为总计
...
//            用户名称列为总计
        ...
        userStatistics.forEach(x -> x.setTotalMoneyStr(MoneyConvertUtil.fenToYuan(String.valueOf(x.getTotalMoney()))));
